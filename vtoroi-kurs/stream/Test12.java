...
public class Test12 {
    public static void main(String[] args) {
        ...
        students.stream().filter(e->e.getAge()>20).limit(2).forEach(System.out::println);// лимит до 2х
        students.stream().filter(e->e.getAge()>20).skip(1).forEach(System.out::println); // скипает первого

        List<Integer> list = students.stream().mapToInt(Student::getAge).boxed().collect(Collectors.toList());
